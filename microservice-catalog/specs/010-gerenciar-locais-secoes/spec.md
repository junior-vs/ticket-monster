...
**Origem**: `microservice-catalog_spec.md` — US-CAT-10, RN07, RN11, RN12.

## Extensões, correções e riscos declarados nesta spec

1. **Capacidade é coluna gerada pelo banco (`GENERATED ALWAYS ... STORED`), não recalculada pela aplicação**: a DDL de origem define `section.capacity` como coluna gerada. Não existe passo de aplicação que "recalcula" — o PostgreSQL deriva o valor automaticamente a partir de `number_of_rows`/`row_capacity`. O payload de escrita MUST NOT aceitar `capacity` como campo de entrada (ver FR-005a).
...
3. **Risco arquitetural não resolvido — ausência de evento de propagação para `microservice-inventory`**: `microservice-inventory_spec.md` declara que `section_snapshot` é sincronizado via consumo de eventos `SectionCreated/Updated` publicados pelo `microservice-catalog`. Nenhum documento define `SectionDeleted` ou `VenueDeleted` (ou uma versão de `SectionUpdated` disparada em alteração de capacidade). Sem esse evento, o inventário pode manter snapshot de uma seção excluída ou com capacidade divergente do catálogo. Esta spec declara esse gap como risco explícito (ver seção "Riscos") e condiciona a entrega completa da feature à existência do evento — não resolve a lacuna arquitetural por conta própria.
4. **Contradição com "Dependências: Nenhuma" do serviço**: o edge case de validar redução de capacidade contra ingressos já vendidos implicaria consulta (síncrona ou de estado replicado) a dados que pertencem a `microservice-inventory`. `microservice-catalog_spec.md` declara explicitamente que o catalog não tem dependências. Esta spec remove a validação síncrona do escopo e documenta a alternativa assíncrona como decisão pendente (ver "Riscos").
...
7. Contrato de chave de cache Redis (FR-010) explicitado.
...
4. **Given** uma requisição de criação de seção, **When** o payload inclui explicitamente um valor para `capacity`, **Then** o sistema DEVE ignorar o campo (ou rejeitar com HTTP 400, conforme política de validação estrita adotada pela equipe) — `capacity` nunca é aceito como entrada, apenas derivado.
...
**Independent Test**: Pode ser testado efetuando requisições HTTP PUT/PATCH nas APIs de Venue ou Section e confirmando que a capacidade é derivada automaticamente pelo banco após a alteração de `number_of_rows`/`row_capacity`.
...
1. **Given** uma seção sem `TicketPrice`/vendas conhecidas pelo `microservice-catalog` (este serviço não possui visibilidade de vendas — ver "Riscos"), **When** o Administrador solicita a exclusão via HTTP DELETE, **Then** o sistema deve remover a seção e retornar HTTP 204 (No Content).
...
- **Alteração de capacidade de seções com ingressos já vendidos**: esta feature NÃO realiza validação síncrona contra o estado de vendas do `microservice-inventory`, por este ser declarado como serviço sem dependências (`microservice-catalog_spec.md`, "Dependências: Nenhuma"). A alteração de `number_of_rows`/`row_capacity` é aceita no `catalog_db` independentemente do estado de vendas; a reconciliação com `inventory` depende de evento de propagação ainda não especificado (ver "Riscos").
...
- **FR-005a**: O sistema MUST NOT aceitar `capacity` como campo de entrada em payloads de criação ou alteração de seção. Se recebido, o valor MUST ser ignorado pela aplicação antes de montar o comando de escrita (a tentativa de gravar diretamente na coluna gerada resultaria em erro de banco, não em um HTTP 400 tratado).
...
- **FR-010**: O sistema DEVE manter cache-aside em Redis para leitura de Venue/Section sob as chaves `catalog:venue:{id}` e `catalog:venue:{id}:sections`, invalidadas de forma síncrona sempre que um Venue ou Seção for alterado ou excluído. Contrato de chave novo, não definido em `arquitetura-solucao.md` seção 10.
...
- **Venue (Entidade Raiz do Agregado)**:
  ...
  - `addressLine`: String (Logradouro e número — nullable, sem RN de formato).
  - `city`: String (Cidade — nullable).
  - `state`: String (Estado/UF — nullable).
  - `postalCode`: String (CEP/Código postal — nullable, sem RN de formato).
...
- **Section (Entidade Filha de Venue)**:
  ...
  - `capacity`: Int (Somente leitura — coluna gerada pelo banco: `numberOfRows * rowCapacity`; não é campo de entrada em nenhuma operação de escrita).
...
- **SC-003**: 100% das tentativas de enviar `capacity` como campo de entrada são neutralizadas (ignoradas ou rejeitadas) sem gerar inconsistência entre valor informado e valor persistido.
...
- **Ausência de evento de propagação `SectionDeleted`/`SectionUpdated`/`VenueDeleted`**: `microservice-inventory_spec.md` documenta apenas o consumo de `SectionCreated/Updated` e `PerformanceScheduled` para popular `section_snapshot`/`performance_snapshot`. Não há especificação de evento para exclusão de seção/Venue nem para redução de capacidade. Sem isso, `inventory` pode reter snapshot de uma seção inexistente ou com capacidade desatualizada no `catalog_db`, criando risco de venda de assentos que não existem mais fisicamente. **Recomendação**: tratar como item para ADR complementar antes da implementação de FR-011 em produção; até lá, a exclusão de seção/Venue nesta feature afeta apenas `catalog_db`.
- **Validação de capacidade contra vendas existentes não é responsabilidade deste serviço**: dado que `microservice-catalog` declara "Dependências: Nenhuma", esta feature não implementa nenhuma checagem síncrona ou assíncrona contra `seat_ledger`/`ticket_price` do `microservice-inventory` antes de permitir redução de `row_capacity`/`number_of_rows`. Se essa validação for exigida pelo negócio, é uma capacidade nova a ser modelada como consumo de evento pelo `inventory` (rejeitando ou sinalizando a divergência), não como chamada síncrona do `catalog`.
...
- O cache das estruturas físicas de locais é mantido em Redis para acelerar a consulta pública de detalhes de locais (US-CAT-05), sob as chaves propostas em FR-010 — contrato novo, não herdado de `arquitetura-solucao.md` seção 10.
...
| FR-003 | RN11 | — |
| FR-004, FR-005, FR-005a | RN12; DDL `section.capacity GENERATED ALWAYS ... STORED` | Corrigido: capacidade nunca é entrada nem cálculo de aplicação |
| FR-006, FR-007 | DDL `show.venue_id ON DELETE RESTRICT`, `section.venue_id ON DELETE CASCADE` | — |
| FR-008 | `arquitetura-solucao.md` seção 8 | RFC 7807 |
| FR-010 | Extensão local (não herdada) | Chave de cache não definida em seção 10 |
| FR-011 / US5 | Gap identificado — não coberto por `microservice-catalog_spec.md` | Exclusão individual de seção; depende de evento de propagação não especificado |
| Risco de propagação `inventory` | `microservice-inventory_spec.md` (Notas de Operação, sincronização de snapshots) | Gap arquitetural cross-serviço, não resolvido nesta feature |
| Remoção de validação síncrona de vendas | `microservice-catalog_spec.md`, "Dependências: Nenhuma" | Contradição identificada e removida do escopo |