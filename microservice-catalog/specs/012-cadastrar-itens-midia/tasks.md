...
- Sources: `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/`
...
- [ ] T001 Criar estrutura de pacotes do agregado `catalog.media.*` (domain, application, adapter/in/rest/dto, adapter/in/rest/exception, adapter/out/persistence/entity, adapter/out/cache, adapter/out/urlcheck) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/`
...
- [ ] T004 [P] Criar entidade de DOMÍNIO PURO `MediaTypeCatalog` (tabela de domínio, SEM annotations JPA/ORM) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/MediaTypeCatalog.java`: campos `code String (PK, ex. "IMAGE")`, `description String`, `enabled boolean`. Construtor validando code not blank.
- [ ] T005 [P] Criar entidade de DOMÍNIO PURO `MediaItem` (aggregate root, SEM annotations JPA/ORM) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/MediaItem.java`: campos `id UUID`, `mediaTypeCode String (FK p/ MediaTypeCatalog)`, `url String (2048)`, `cachedFileName String (nullable, 255)`, `fallbackApplied boolean`, `createdAt OffsetDateTime`. Invariantes: `mediaTypeCode` not blank; `url` not blank; `fallbackApplied` default false se não informado.
- [ ] T006 [P] Criar `UrlValidator` utilitário de DOMÍNIO PURO (sem dependências) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/UrlValidator.java`:
...
- [ ] T007 [P] Criar fábrica estática `MediaItemFactory` de DOMÍNIO PURO em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/MediaItemFactory.java`:
...
- [ ] T008 [P] Criar porta de saída `MediaItemRepositoryPort` (interface SEM dependências) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/MediaItemRepositoryPort.java`: métodos `Uni<MediaItem> save(MediaItem item)`, `Uni<MediaItem> findById(UUID id)`, `Uni<Page<MediaItem>> findAll(int page, int size)`, `Uni<Boolean> existsByUrl(String url, UUID excludeIdIfAny)`, `Uni<Void> deleteById(UUID id)`.
- [ ] T009 [P] Criar porta de saída `MediaTypeCatalogReaderPort` (interface SEM dependências) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/MediaTypeCatalogReaderPort.java`: métodos `Uni<Optional<MediaTypeCatalog>> findByCodeAndEnabled(String code)` (retorna vazio se desabilitado ou inexistente).
- [ ] T010 [P] Criar porta de saída `MediaItemCachePort` (interface SEM dependências Redis) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/MediaItemCachePort.java`: `Uni<MediaItem> get(UUID id)`, `Uni<Void> set(MediaItem item, Duration ttl)`, `Uni<Void> evict(UUID id)`.
- [ ] T011 [P] Criar porta de saída `RemoteUrlValidatorPort` (interface SEM dependências Vert.x/RestClient) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/RemoteUrlValidatorPort.java`: `Uni<Boolean> isAccessible(String url)` — retorna `true` se HEAD/GET remoto respondeu OK (<400); retorna `false` SEQUER se qualquer exceção/timeout/erro (4xx/5xx/network/timeout). **NÃO** lançar exceção p/ qualquer falha de rede.
- [ ] T012 [P] Criar Panache entity `MediaTypeCatalogEntity` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/out/persistence/entity/MediaTypeCatalogEntity.java`: mapear `catalog.media_type_catalog`, colunas `code VARCHAR(30) PK`, `description VARCHAR(120) NOT NULL`, `enabled BOOLEAN NOT NULL DEFAULT TRUE`. Usar Hibernate Reactive Panache.
- [ ] T013 [P] Criar Panache entity `MediaItemEntity` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/out/persistence/entity/MediaItemEntity.java`: mapear `catalog.media_item`, colunas `id UUID PK`, `media_type_code VARCHAR(30) FK catalog.media_type_catalog.code`, `url VARCHAR(2048) NOT NULL`, `cached_file_name VARCHAR(255) NULL`, `fallback_applied BOOLEAN NOT NULL DEFAULT FALSE`, `created_at TIMESTAMPTZ NOT NULL DEFAULT now()`. Unique constraint `uq_media_item_url` em `url`. Many-to-One `mediaTypeCatalog` ou apenas coluna `mediaTypeCode` (simplificar já que MediaTypeCatalog é tabela de domínio read-only para esta feature).
- [ ] T014 Implementar adapter persistência `PanacheMediaItemRepository` implementa `MediaItemRepositoryPort` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/out/persistence/PanacheMediaItemRepository.java`: mapper MediaItem ↔ MediaItemEntity. `existsByUrl`: native query `SELECT 1 FROM catalog.media_item WHERE url = $1 [AND id <> $2]` case-sensitive ou via Panache. Capturar `PersistenceException` ou `ConstraintViolationException` de `uq_media_item_url` e relançar como exceção de domínio `DuplicateMediaUrlException` (ou deixar exceção para mapper REST tratar).
- [ ] T015 Implementar adapter persistência `PanacheMediaTypeCatalogReader` implementa `MediaTypeCatalogReaderPort` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/out/persistence/PanacheMediaTypeCatalogReader.java`: findById Panache + filtro `enabled = TRUE`.
- [ ] T016 Implementar adapter cache `RedisMediaItemCache` implementa `MediaItemCachePort` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/out/cache/RedisMediaItemCache.java`: chave `catalog:media-item:{id}` (FR-007). Serializar `MediaItem` para JSON via `ObjectMapper`; desserializar de volta. TTL via `@ConfigProperty(name = "catalog.media.cache.ttl-seconds") Duration`. Usar `ReactiveRedisDataSource` Quarkus ou `RedisClient` reativo.
- [ ] T017 Implementar adapter validador remoto `VertxRemoteUrlValidator` implementa `RemoteUrlValidatorPort` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/out/urlcheck/VertxRemoteUrlValidator.java`:
...
- [ ] T026 [P] [US1] Criar DTOs REST em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/in/rest/dto/`:
...
- [ ] T027 [P] [US1] Implementar caso de uso `CreateMediaItemUseCase` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/application/CreateMediaItemUseCase.java`:
...
- [ ] T028 [P] [US1] Implementar caso de uso `GetMediaItemUseCase` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/application/GetMediaItemUseCase.java`:
...
- [ ] T029 [P] [US1] Implementar caso de uso `ListMediaItemsUseCase` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/application/ListMediaItemsUseCase.java`: paginação `page` base 0, `size` (default 20, validação 1..100). Apenas `repo.findAll(page, size)`; listagens NÃO são cacheadas nesta feature.
- [ ] T030 [US1] Implementar exceções de domínio e ExceptionMapper RFC 7807 em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/in/rest/exception/MediaItemExceptionMapper.java`:
  - Exceções de domínio a criar em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/domain/exception/`:
...
- [ ] T031 [US1] Implementar `MediaItemAdminResource` (admin ROLE_ADMIN) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/in/rest/MediaItemAdminResource.java`:
...
- [ ] T032 [US1] Implementar `MediaItemPublicResource` (público SEM auth) em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/adapter/in/rest/MediaItemPublicResource.java`:
...
- [ ] T039 [P] [US3] Implementar caso de uso `DeleteMediaItemUseCase` em `../../src/main/java/br/vsjr/labs/ticketmonster/catalog/media/application/DeleteMediaItemUseCase.java`:
